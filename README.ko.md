...
**핵심:** 호출할 때마다 **LS 인스턴스를 새로 spawn**합니다. IDE 창이 없어도 됩니다.
...
│                   antigravity-cli                    │
│                                                     │
│  argv → config → state.vscdb (인증 + 모델)           │
│  → metadata.bin → fake extension server              │
│  → LS spawn → USS subscribe (인증 핸드오프)           │
...
└──────────────────────┬──────────────────────────────┘
                       ...
                       ▼
┌─────────────────────────────────────────────────────┐
│         language_server_macos_arm (LS 바이너리)       │
│         Antigravity.app 내장                         │
└─────────────────────────────────────────────────────┘
```
...
3. **Fake extension server**가 역방향 RPC 처리 (USS 구독, heartbeat)
4. spawn된 LS에 **ConnectRPC** over HTTPS (자체 서명 `cert.pem`)로 통신
5. agent state 업데이트를 스트리밍하고, 대화 진행에 따라 trajectory steps를 가져옴
6. 후처리: `UpdateConversationAnnotations` + `trajectorySummaries` hydration으로 IDE에서 보이게 함

**Bridge Extension 불필요. IDE 창 불필요.** LS 바이너리와 직접 통신합니다.
...
- 호출할 때마다 **LS 인스턴스를 새로 spawn**합니다 (1:1 one-shot 모델).
